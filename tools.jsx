// tools.jsx — Bread / Cocktail / Price screens
...
function BreadScreen({ onBack, store }) {
...
function CocktailScreen({ onBack, store }) {
...
function CocktailMixing() {
  ...
  // Glass fill: alcohol portion + aux portion
...
function CocktailGlass({ alc = 0, aux = 0, mix = 0 }) {
  // a tall tumbler glass with liquid filling from bottom
  const totalFill = Math.min(100, alc + aux + mix);
  const liquidH = 80; // height of liquid area in svg units
  const baseY = 110; // bottom of liquid
  const fillH = (totalFill / 100) * liquidH;
  const topY = baseY - fillH;
  const alcH = (alc / 100) * liquidH;
  const auxH = (aux / 100) * liquidH;
  // layer order from bottom: alc, aux, mix
  return (
...
function PriceScreen({ onBack, store }) {
...
window.BreadScreen = BreadScreen;
window.CocktailScreen = CocktailScreen;
