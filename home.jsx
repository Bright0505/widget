// home.jsx — Splash + Home (with Favorites + History tabs)
...
function StarIcon({ filled = false, size = 18 }) {
  return (
  ...
  return (
    <div className={`tool-card ${size} ${extraClass}`} role="button" tabIndex={0}
      ...
      <div style={{position:'absolute', top:10, right:10, zIndex:2}}
        ...
        <div className={`icon-btn ${isFav ? 'on' : ''}`}
...
function Home({ onPick, layout = 'mixed', store }) {
  const [tab, setTab] = useState('tools'); // tools | history
          ...
          <div>
            ...
            {layout === 'list' && (
              <div className="list-stack">
                {TOOLS.map(t => (
                  <div key={t.id} className="list-row" role="button" tabIndex={0}
                    onClick={() => onPick(t.id)}>
                ...
                {history.map(h => {
                  ...
                  return (
                    <div key={h.id} className="history-row">
                      ...
                      <div className="h-meta" role="button" tabIndex={0}
                        style={{cursor:'pointer'}}
                        onClick={() => onPick(h.toolId)}>
