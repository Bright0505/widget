...
### 🥖 法國麵包配方計算器 (`french-bread-calculator.html`)
**功能**：根據麵粉重量自動計算烘焙材料用量
- 輸入：麵粉重量
- 輸出：水、鹽、酵母用量 + 烘焙百分比 + 總重量
- 特色：T55/T65 兩種配方切換
- 製作提示：發酵時間、烘烤溫度等專業建議
...
### 🎨 視覺設計
- **標準字型系統**：採用 Material Design 規範，H1(2rem)、H2(1.5rem)、Body(1rem) 等標準階層
- **統一間距規則**：使用 4px、8px、12px、16px、24px 倍數系統，確保視覺協調
- **一致性**：統一的設計語言和配色
- **漸層主題**：每個工具都有專屬漸層色
- **清晰層級**：明確的資訊層級和視覺引導
- **即時反饋**：按鈕點擊、表單驗證等互動反饋
...
### 📈 最近更新
- **✅ 生活記事狀態恢復** (2025/05)：生活記事現在支援點擊後直接進入工具並恢復計算狀態，就像分享功能一樣；調酒工具的輔料信息現已正確保存並在摘要中顯示
- **✅ 法國麵包配方計算器** (2025/10)：新增 T55/T65 麵包配方計算工具，以麵粉重量為基礎自動計算材料用量
- **✅ 字型系統正規化** (2024/08)：採用標準設計規範統一字體大小與間距
